...
func (d *Dispatcher) AddFunc(funcName string, handler interface{}, caller ...interface{}) {
	...
	if len(caller) > 0 && caller[0] != nil {
		...
		if reflect.TypeOf(handler) == reflect.TypeOf("") {
			f.Handler = reflect.ValueOf(mCaller).MethodByName(handler.(string))
			f.IsMethod = true
		} else {
			f.Caller = reflect.ValueOf(mCaller)
